  ...
  assignSecretSantas: async (req, res) => {
    ...
    //   console.log(assignments);
      ...
      for (const assignment of assignments) {
        ...
        console.log(participant, secretSantaId);

        await Participant.updateOne({ _id: participantId }, { $set: { assignedRecipient: secretSantaId } });
        await participant.save();
      }
  
      res.status(200).json({ message: 'Secret Santas assigned successfully' });
    } catch (error) {
      res.status(500).json({ error: `Failed to assign Secret Santas.`, details: `${error}` });
    }
  },
